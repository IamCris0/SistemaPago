const routing = {
  ...
  handleInitialURL() {
    ...
    if (params.product) {
      ...
      const checkProducts = setInterval(() => {
        if (state.products && state.products.length > 0) {
          ...
          if (product) {
            ...
            if (window.mawewe && window.mawewe.ui) {
              window.mawewe.ui.showNotification(
      ...
      return; // No procesar otros parámetros si hay producto
    }

    // PRIORIDAD 2: Búsqueda (ignora categorías)
    if (params.search) {
      ...
      return; // No procesar categorías si hay búsqueda
    }

    // PRIORIDAD 3: Categoría y subcategoría
    if (params.category) {
          ...
          setTimeout(() => {
            document.querySelectorAll(".filter-button").forEach((btn) => {
              ...
              if (
                ...
                (btnText === "todos" && params.category === "all")
              ) {
